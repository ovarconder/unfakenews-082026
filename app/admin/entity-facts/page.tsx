...
import { Plus, Trash2, Save, AlertCircle, Check, Eye, EyeOff, X, RefreshCw, Database, Search } from "lucide-react";
...
// Article Selector (fetches from API)
  ...
  return (
    <div>
      ...
      >
        ...
        {filtered.length === 0 && (
          <option disabled className="text-white/30">ไม่พบบทความ</option>
        )}
      </select>
      )}
    </div>
...
// Quick Fact Row (no labelEn)
function QuickFactRow({
  index, fact, onChange, onRemove,
}: {
  index: number;
  ...
  return (
    <div className="flex items-start gap-2 p-3 rounded-lg bg-white/[0.03] border border-white/5 group">
      <div className="flex-1 space-y-2">
        <input type="text" value={fact.label} onChange={(e) => onChange(index, "label", e.target.value)}
        ...
        <textarea value={fact.value} onChange={(e) => onChange(index, "value", e.target.value)}
          placeholder="ค่า..." rows={2}
...
export default function EntityFactsManagerPage() {
  ...
  // Fetch all articles and find those with entity facts
  useEffect(() => {
    fetch("/api/admin/articles")
      .then((r) => r.json())
      .then((data) => {
  ...
  const handleSelectArticle = useCallback((slug: string, article: ArticleMaster) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (listData.articles) {
        setArticlesWithFacts(listData.articles.filter((a: any) => a.entityName || a.quickFacts));
      ...
      <div className="border-b border-white/10 bg-gradient-to-r from-[#0f1f3a] to-[#162545]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-base sm:text-2xl font-bold text-white">Entity Facts Manager</h1>
              <p className="text-white/40 text-sm mt-1">
                จัดการ Entity Name และ Quick Facts สำหรับบทความ Wiki-Style (บันทึกลงฐานข้อมูล)
              </p>
            </div>
          ...
          <div className="mt-3 flex items-center gap-2 text-xs text-white/40">
            <span>บทความที่มี Entity Facts: <strong className="text-amber-300">{articlesWithFacts.length}</strong> รายการ</span>
            {selectedArticle && (
            ...
            <div className="rounded-xl bg-gradient-to-br from-[#0f1f3a] to-[#162545] border border-white/10 p-4">
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto">
                {loadingList ? (
                  ...
                  <p className="text-white/20 text-xs text-center py-4">ยังไม่มี Entity Facts</p>
                ) : (
          ...
          <div className="lg:col-span-2 space-y-6">
            {!selectedSlug ? (
              <div className="rounded-xl bg-gradient-to-br from-[#0f1f3a] to-[#162545] border border-white/10 p-8 text-center">
                ...
                <p className="text-white/40 text-sm">เลือกบทความจากด้านซ้ายเพื่อเริ่มแก้ไข Entity Facts</p>
              </div>
            ) : (
              <>
                {/* Entity Info */}
                ...
                <div className="rounded-xl bg-gradient-to-br from-[#0f1f3a] to-[#162545] border border-white/10 p-5 space-y-4">
                  ...
                  <div className="space-y-3">
                    {facts.map((fact, index) => (
                      <QuickFactRow key={index} index={index} fact={fact}
                        onChange={handleFactChange} onRemove={removeFactRow} />
                    ))}
                    {facts.length === 0 && (
                      <p className="text-white/20 text-xs text-center py-3">ยังไม่มี Quick Facts — กด "เพิ่มแถว"</p>
                    )}
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  {saveStatus === "saved" && (
                    <span className="text-xs text-emerald-400 flex items-center gap-1">
                      <Check size={12} /> บันทึกลงฐานข้อมูลแล้ว
                    </span>
                  )}
                </div>
                ...
                {preview && previewJson && (
                  <div className="rounded-xl bg-[#0a0f1a] border border-white/10 p-4">
                    <h3 className="text-white/50 text-xs font-mono mb-2">JSON Preview (Entity Facts)</h3>
