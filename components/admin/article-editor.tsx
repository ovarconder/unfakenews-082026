...
export function ArticleEditor({ initialData, onSave, onDelete }: ArticleEditorProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // ================================================================
      // 🔄 Auto-translate — หลังบันทึกบทความสำเร็จ
      // ================================================================
      setTranslating(true);
      // ใช้ AbortController เพื่อป้องกันการเรียกซ้ำซ้อน
      const translateAbort = new AbortController();

      try {
        // 1. ดึง tier config จาก API
        const tiersRes = await fetch("/api/settings/tiers", { signal: translateAbort.signal });
        const tiersData = await tiersRes.json();
        const tierConfig: Record<string, "0" | "1" | "2"> = tiersData.tiers || {};

        // 2. ตรวจสอบว่าควรแปลเฉพาะ fields ที่เปลี่ยนแปลง (ถ้ามี initialData)
        ...
        // 3. รายการ locales ที่ต้องแปล (ไม่รวม "th" และ Tier 0)
        const localesToTranslate = Object.entries(tierConfig)
          .filter(([locale, tier]) => locale !== "th" && tier !== "0")
          .map(([locale, tier]) => ({ locale, tier }));

        setTranslateProgress(`กำลังแปล ${localesToTranslate.length} ภาษา...`);

        // 4. แปลทีละภาษา (sequential) เพื่อให้ติดตาม progress ได้
        //    ✅ เช็ค result.success จริง — ไม่นับรวมที่ล้มเหลว
        let doneCount = 0;
        let errorCount = 0;
        const errorLocales: string[] = [];
        for (const { locale, tier } of localesToTranslate) {
          if (translateAbort.signal.aborted) break;

          // Tier 2: ไม่รวม content (เฉพาะ head fields)
          // Tier 1: แปลทุก dirty field
          const fieldsForLocale = dirtyFields.length > 0
            ? (tier === "2"
                ? dirtyFields.filter(f => f !== "content")
                : dirtyFields
              )
            : undefined;
          ...
          try {
            const res = await fetch("/api/translate-new", {
              ...
              body: JSON.stringify({
                slug,
                locale,
                dirtyFields: fieldsForLocale && fieldsForLocale.length > 0 ? fieldsForLocale : undefined,
              }),
            });
            ...
            if (result.success) {
              ...
              errorCount++;
              errorLocales.push(locale.toUpperCase());
              console.warn(`[Auto-translate] ${locale}: ${result.error || "unknown error"}`);
            }
          } catch (err: any) {
            if (err.name === "AbortError") break;
            errorCount++;
            errorLocales.push(locale.toUpperCase());
            console.error(`[Auto-translate] ${locale}: fetch error`, err);
          }

          setTranslateProgress(
            errorCount > 0
              ? `กำลังแปล: ✅${doneCount} ⚠️${errorCount}/${localesToTranslate.length} (${locale.toUpperCase()})`
              : `กำลังแปล: ${doneCount + errorCount}/${localesToTranslate.length} (${locale.toUpperCase()})`
          );
        }

        if (!translateAbort.signal.aborted) {
          if (errorCount === 0) {
            setTranslateProgress(`✅ แปลครบ ${doneCount}/${localesToTranslate.length} ภาษา`);
            addNotification({
              type: "translation_done",
              title: `✅ แปล "${title}"`,
              message: `เสร็จ ${doneCount}/${localesToTranslate.length} ภาษา (Auto-translate หลังบันทึก)`,
              slug,
              category,
            });
          } else if (doneCount > 0) {
            setTranslateProgress(`⚠️ แปล ${doneCount} ภาษา — มีข้อผิดพลาด ${errorCount} ภาษา`);
            addNotification({
              type: "translation_done",
              title: `⚠️ แปล "${title}" ไม่สมบูรณ์`,
              message: `สำเร็จ ${doneCount}/${localesToTranslate.length} ภาษา — ผิดพลาด ${errorCount} ภาษา (${errorLocales.join(", ")})`,
              slug,
              category,
            });
          } else {
            setTranslateProgress(`❌ แปล "${title}" ไม่สำเร็จทั้งหมด ${errorCount} ภาษา`);
            addNotification({
              type: "translation_error",
              title: `❌ แปล "${title}" ล้มเหลว`,
              message: `ผิดพลาด ${errorCount}/${localesToTranslate.length} ภาษา — ${errorLocales.join(", ")}`,
              slug,
              category,
            });
          }
        ...
        if (err.name === "AbortError") {
          setTranslateProgress("⏹️ การแปลถูกหยุด (เปลี่ยนหน้า)");
        } else {
          console.warn("[Auto-translate] Failed to fetch tiers or translate:", err);
          setTranslateProgress("⚠️ แปลไม่สมบูรณ์ — กรุณากด 'แปลภาษา' ด้วยตนเอง");
          addNotification({
            type: "translation_error",
            title: `❌ แปล "${title}" ไม่สมบูรณ์`,
            message: "เกิดข้อผิดพลาดขณะ Auto-Translate — กรุณากด 'แปลภาษา' ด้วยตนเอง",
            slug,
            category,
          });
        }
        ...
        <div className="flex items-center gap-3 justify-end">
          ...
          {initialData && (
            <button
              onClick={async () => {
                ...
                setTranslating(true);
                setTranslateProgress("กำลังดึงการตั้งค่า...");
                try {
                  ...
                  const errorLocales: string[] = [];
                  for (const { locale, tier } of localesToTranslate) {
                    // Tier 2: ส่ง dirtyFields เฉพาะ head fields (route จะจัดการ tier 2 เอง)
                    // Tier 1: ไม่ส่ง dirtyFields = translate all
                    const fieldsForLocale = tier === "2"
                    ...
                    setTranslateProgress(
                      errorCount > 0
                        ? `กำลังแปล: ✅${done} ⚠️${errorCount}/${localesToTranslate.length} (${locale.toUpperCase()})`
                        : `กำลังแปล: ${done + errorCount}/${localesToTranslate.length} (${locale.toUpperCase()})`
                    );
                  }
                  if (errorCount === 0) {
                    setTranslateProgress(`✅ แปลครบ ${done}/${localesToTranslate.length} ภาษา`);
                    addNotification({
                      type: "translation_done",
                      title: `✅ แปล "${title}"`,
                      message: `เสร็จ ${done}/${localesToTranslate.length} ภาษา (Manual)`,
                      slug,
                      category,
                    });
                  } else if (done > 0) {
                    setTranslateProgress(`⚠️ แปล ${done} ภาษา — มีข้อผิดพลาด ${errorCount} ภาษา`);
                    addNotification({
                      type: "translation_done",
                      title: `⚠️ แปล "${title}" ไม่สมบูรณ์`,
                      message: `สำเร็จ ${done}/${localesToTranslate.length} ภาษา — ผิดพลาด ${errorCount} ภาษา (${errorLocales.join(", ")})`,
                      slug,
                      category,
                    });
                  } else {
                    setTranslateProgress(`❌ แปล "${title}" ไม่สำเร็จทั้งหมด`);
                    addNotification({
                      type: "translation_error",
                      title: `❌ แปล "${title}" ล้มเหลว`,
                      message: `ผิดพลาด ${errorCount}/${localesToTranslate.length} ภาษา`,
                      slug,
                      category,
                    });
                  }
                } catch {
                  setTranslateProgress("⚠️ เกิดข้อผิดพลาด");
                  addNotification({
                    type: "translation_error",
                    title: `❌ แปล "${title}" ล้มเหลว`,
                    message: "เกิดข้อผิดพลาดขณะแปล — ลองใหม่อีกครั้ง",
                    slug,
                    category,
                  });
                }
                setTimeout(() => { setTranslating(false); setTranslateProgress(null); }, 4000);
              }}
          ...
          {onDelete && (
            <button
              onClick={onDelete}
              disabled={translating}
              className="flex items-center gap-2 px-4 py-2.5 rounded-lg bg-red-500/10 border border-red-500/20 text-red-400 hover:bg-red-500/20 transition-all text-sm disabled:opacity-50"
            >
